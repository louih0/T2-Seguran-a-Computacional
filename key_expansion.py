...
def key_expansion(english_key, n_rounds):
    s_box = [
    ...
    def english_key_to_hex(key):
        return [hex(ord(c)) for c in key]

    key_size = 16
    hex_key = english_key_to_hex(english_key)
    words = [hex_key[i:i + 4] for i in range(0, key_size, 4)]

    for round in range(n_rounds):
        func_last = deque(words[-1])
        func_last.rotate(-1)
        func_last = list(func_last) 
        for i in range(len(func_last)):
            word = '0x' + func_last[i][2:].zfill(2)
            sub_word = s_box[int(word[2],16)][int(word[3],16)]
            func_last[i] = hex(sub_word)
        func_last[0] = hex(int(func_last[0],16) ^ int(rcon[round],16))    
        words.append([hex(int(pair[0],16) ^ int(pair[1],16)) for pair in zip(words[4*round], func_last)])
        for i in range(1,4):
            words.append([hex(int(pair[0],16) ^ int(pair[1],16)) for pair in zip(words[-1], words[round*4+i])])
    return words
